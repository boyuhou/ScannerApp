...
if __name__ == '__main__':
    with iq.ConnConnector([bar_conn]) as conn:
        # bar_conn.watch(symbol=ticker, interval_len=60 * 60 , interval_type='s', update=10, lookback_bars=50)
        bar_conn.watch(symbol='EURUSD.FXCM', interval_len=5 * 60, interval_type='s', update=1, lookback_bars=60)
